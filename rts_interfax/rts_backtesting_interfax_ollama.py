...
cache_file = Path(
    fr'C:\Users\Alkor\PycharmProjects\beget_rss\{ticker_lc}_{rss_provider}'
    fr'\{ticker_lc}_embeddings_{rss_provider}_ollama.pkl')
...
result_file = Path(
    fr'C:\Users\Alkor\PycharmProjects\beget_rss\{ticker_lc}_{rss_provider}'
    fr'\{ticker_lc}_backtest_results_{rss_provider}_ollama.xlsx')

# Настройка логирования: вывод в консоль и в файл, файл перезаписывается
log_file = Path(
    fr'C:\Users\Alkor\gd\predict_ai\{ticker_lc}_{rss_provider}_ollama\log'
    fr'\{ticker_lc}_backtesting_{rss_provider}_ollama.txt')
...
def main(max_prev_files: int = 8):
    ...
    for test_idx in range(min_prev_files, len(documents)):
        ...
        if len(prev_cache) < min_prev_files:
            logger.error(
                f"Недостаточно предыдущих документов для даты {test_date}: {len(prev_cache)}")
            continue
