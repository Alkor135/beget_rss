"""
Скрипт для чтения базы данных котировок и новостей из нескольких файлов БД, формирования
markdown-файлов с заголовками новостей.
Сохраняет не более указанных последних интервалов в формате markdown с метаданными.
Использует базу данных котировок с дневными свечами, сформированными из минутных данных
с 21:00 МСК предыдущей сессии.
...
rss_provider: str = 'interfax'  # Поставщик новостей
...
log_file = Path(
    fr'C:\Users\Alkor\gd\predict_ai\{ticker_lc}_{rss_provider}_ollama\log\{ticker_lc}_21_00_db_{rss_provider}_month_to_md.txt')
...
def main(
    ...
    db_files = list(db_news_dir.glob(f"rss_news_{rss_provider}_*_*.db"))
    # print(db_files)
    if not db_files:
