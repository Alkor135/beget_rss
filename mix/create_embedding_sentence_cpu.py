...
# Вместо OllamaEmbeddingFunction:
class DeterministicEmbeddingFunction:
    def __init__(self, model_name="BAAI/bge-m3"):
        self.model = SentenceTransformer(model_name)

    def __call__(self, texts):
        # Убедимся, что эмбеддинги float64 и нормализованы
        embeddings = self.model.encode(
            texts,
            convert_to_numpy=True,
            normalize_embeddings=True  # BGE требует нормализации
        )
        # Приводим к float64 для детерминированности при сериализации
