...
if __name__ == '__main__':
    ...
    md_news_dir = Path('c:/Users/Alkor/gd/news_rss_md_no_metadata')

    if not path_db_quote.exists():
        print(f"Ошибка: Файл БД котировок не найден. {path_db_quote}")
        exit()

    if not path_db_news.exists():
        print("Ошибка: Файл БД новостей не найден.")
        exit()
