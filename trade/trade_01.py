...
from QuikPy import QuikPy  # Установите через pip install QuikPy
...
print("Скрипт запущен. Ожидание файлов с предсказаниями...")

while True:
    # Получение текущей даты для имени файла
    today = datetime.date.today().isoformat()  # Формат YYYY-MM-DD
    file_path = r'C:\Users\Alkor\gd\predict_ai\rts_investing_ollama\\' + today + '.txt'
    processed_path = file_path + '_processed'  # Имя для переименованного файла

    if os.path.exists(file_path) and not os.path.exists(processed_path):  # Проверяем, существует ли файл и не обработан ли уже
        print(f"Обнаружен файл: {file_path}. Обработка...")

        # Шаг 1: Проверка подключения к QUIK
        if qp.IsConnected()['data'] != 1:
        ...
        param_ex = qp.GetParamEx(class_code, sec_code, 'STATUS')['data']
        ...
        holding = qp.GetFuturesHolding(firm_id, account, sec_code, 0)['data']  # 0 - открытая позиция
