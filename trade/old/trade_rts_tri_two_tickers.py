...
# Параметры
ticker_close = 'RIU5'  # Торгуемый инструмент закрываемой позиции
quantity_close = '1'  # Количество закрываемых позиций
ticker_open = 'RIZ5'  # Торгуемый инструмент открываемой позиции
quantity_open = '1'  # Количество открываемых позиций
ticker_lc = 'rts'  # Торгуемый инструмент в нижнем регистре
...
if current_predict == 'down' and prev_predict == 'up':
    ...
    trade_content = (
        ...
        f'Тип=Рыночная;'
        f'Класс=SPBFUT;'
        f'Инструмент={ticker_close};'
        f'Цена=0;'
        f'Количество={quantity_close};'
        f'Условие исполнения=Поставить в очередь;'
        f'Комментарий=SPBFUT192yc//TRI//close;'
        ...
        f'Тип=Рыночная;'
        f'Класс=SPBFUT;'
        f'Инструмент={ticker_open};'
        f'Цена=0;'
        f'Количество={quantity_open};'
        f'Условие исполнения=Поставить в очередь;'
        f'Комментарий=SPBFUT192yc//TRI//open;'
        f'Переносить заявку=Нет;'
        f'Дата экспирации={expiry_date};'
        f'Код внешнего пользователя=;\n'
    )
