...
def get_daily_candle(cursor, start: str, end: str) -> tuple:
    ...
    # Запрос для получения OPEN, LOW, HIGH, CLOSE, SECID, LSTTRADE
    query = """
        SELECT 
            ...
            (SELECT SECID FROM Futures WHERE TRADEDATE = (
                SELECT MAX(TRADEDATE) FROM Futures WHERE TRADEDATE BETWEEN ? AND ?
            ) LIMIT 1) AS SECID,
            (SELECT LSTTRADE FROM Futures WHERE TRADEDATE = (
                SELECT MAX(TRADEDATE) FROM Futures WHERE TRADEDATE BETWEEN ? AND ?
            ) LIMIT 1) AS LSTTRADE
        FROM Futures
        WHERE TRADEDATE BETWEEN ? AND ?
    """
    cursor.execute(query, (start, end, start, end, start, end, start, end, start, end))
    result = cursor.fetchone()

    if result and all(result[i] is not None for i in range(6)):
        return (trade_date, result[0], result[1], result[2], result[3], result[4], result[5])
    return None
