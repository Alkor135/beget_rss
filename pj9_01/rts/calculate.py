"""
Скрипт анализирует временные ряды новостных данных с эмбеддингами.
Для каждой записи находит три наиболее похожих по эмбеддингам события из предыдущих дней (по одному на день).
Проверяет, находились ли все три события в верхнем (>perc_75) или нижнем (<perc_25) квантиле по метрике H2.
Если условие выполнено (все три — «высокие» или «низкие»), текущая запись помечается как значимая.
Формируется флаг all_high или all_low в зависимости от направления сигнала.
Считается показатель rez: модуль H2 при совпадении знака и флага, иначе — отрицательный.
Результат сохраняется в Excel с кумулятивной суммой rez_cum для оценки эффективности стратегии.
"""
