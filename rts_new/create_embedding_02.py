"""
Скрипт инкрементально обновляет кэш эмбеддингов для ежедневных markdown-отчётов.
Использует Ollama (локально) с поддержкой моделей bge-m3, qwen3-embedding и embeddinggemma.
Разбивает текст на чанки по токенам, сохраняя семантику параграфов.
Автоматически пропускает неизменённые файлы через сравнение MD5-хэшей.
Генерирует L2-нормализованные эмбеддинги для каждого чанка и сохраняет их в pickle.
Поддерживает ротацию логов — хранит только 3 последних файла.
Результат пригоден для быстрой загрузки в RAG-системах.
"""
