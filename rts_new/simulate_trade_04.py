...
WHITE_LIST_PATH = Path(__file__).parent / "white_list.pkl"  # Путь к white-list
...
# white_list: { headline -> stats }
if WHITE_LIST_PATH.exists():
    with open(WHITE_LIST_PATH, "rb") as f:
        WHITE_LIST = pickle.load(f)
    logging.info(f"🤍 White-list loaded: {len(WHITE_LIST)} headlines")
else:
    WHITE_LIST = {}
    logging.info("🤍 White-list initialized empty")

WHITE_ALPHA = 0.15   # сила усиления (15%)
...
def white_list_score(chunk_text: str) -> float:
    """Возвращает 1.0 если в тексте чанка есть заголовок из white-list, иначе 0.0"""
    if not chunk_text or not WHITE_LIST:
        return 0.0

    for h in chunk_text.split("\n"):
        key = normalize_headline(h)
...
def normalize_headline(h: str) -> str:
    """Нормализация заголовка для white-list."""
...
def chunks_similarity_with_explain(
    ...
    for idx in top_idx_sorted:
        ...
        base_sim = float(S[ia, ib])

        wl_a = white_list_score(text_a)
        wl_b = white_list_score(text_b)

        weight = 1.0 + WHITE_ALPHA * max(wl_a, wl_b)
        sim = base_sim * weight
...
def compute_max_k(
    ...
    for i in range(start_pos, len(df)):
        ...
        top_candidates = np.argsort(similarities)[-TOP_N:]
        ...
        EXPLAIN_STORE[k].append({
            ...
            "score": float(best_score),  # ← ВАЖНО
            "explained_score": float(best_score),  # ← теперь одно и то же
            "pairs": pairs,
        ...
        profitable = np.sign(body_cur) == np.sign(body_prev)
        ...
        if i % 50 == 0:
            logging.info(
                f"[DELTA] {dates[i].date()} | "
                f"best_j={dates[best_j].date()} | "
                f"score={best_score:.4f} | "
                f"white_list_size={len(WHITE_LIST)}"
            )

        # === UPDATE WHITE-LIST (STRICTLY AFTER RESULT) ===
        if profitable and pairs:
            for p in pairs:
                for h in p["text_a"].split("\n"):
                    key = normalize_headline(h)
                    if not key:
                        continue
                    if key not in WHITE_LIST:
                        WHITE_LIST[key] = {
                            "count": 1,
                            "total_pl": abs(body_cur)
                        }
                    else:
                        WHITE_LIST[key]["count"] += 1
                        WHITE_LIST[key]["total_pl"] += abs(body_cur)
...
def main(path_db_day, cache_file):
    ...
    # === SAVE WHITE-LIST ===
    try:
        with open(WHITE_LIST_PATH, "wb") as wf:
            pickle.dump(WHITE_LIST, wf)
        logging.info(f"🤍 White-list saved: {len(WHITE_LIST)} headlines")
    except Exception as e:
        logging.error(f"Failed to save white-list: {e}")
