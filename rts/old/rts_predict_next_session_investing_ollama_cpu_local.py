...
Изменено: использование sentence-transformers вместо Ollama для модели bge-m3.
"""
import os
...
def cache_embeddings(documents, cache_file, model_name):
    """Кэширует эмбеддинги новых и изменённых файлов с sentence-transformers."""
    model = SentenceTransformer(model_name, device='cpu')  # Принудительно CPU для детерминизма
    ...
    if new_docs:
        ...
        batch_size = 1  # Можно увеличить для скорости, но оставляем как в оригинале
...
def main(max_prev_files: int = 7):
    ...
    cache = cache_embeddings(documents, cache_file, model_name)
