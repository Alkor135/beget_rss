...
def cosine_similarity(vec1, vec2):
    ...
    # Используем numpy для более эффективных вычислений
...
def cache_embeddings(documents, cache_file, model_name, url_ai):
    ...
    if new_docs:
        ...
        print(
            f"{timestamp} - Вычисление эмбеддингов для {len(new_docs)} новых/изменённых файлов: {', '.join(new_files)}")

        batch_size = 1
        for i in range(0, len(new_docs), batch_size):
            batch_docs = new_docs[i:i + batch_size]
            batch_contents = [doc.page_content for doc in batch_docs]
            print(f"Обработка батча {i // batch_size + 1} с {len(batch_contents)} документами")
            try:
...
def main(max_prev_files: int = 7):
    ...
    output_file.unlink(missing_ok=True)  # Удаляем существующий файл, если он есть
