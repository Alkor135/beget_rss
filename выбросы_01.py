...
Он вычисляет абсолютное изменение цены за 15 минут: разницу между ценой открытия и ценой закрытия через 14 свечей.
Для каждой текущей свечи собираются аналогичные 15-минутные изменения за 10 предыдущих торговых дней.
На основе этих данных определяется, является ли текущее изменение выбросом по методу IQR.
Выброс — это значение, выходящее за пределы 1.5 межквартильного размаха от Q1 или Q3.
Результатом является список свечей, на которых наблюдался аномально высокий или низкий 15-минутный пробег.
Итог сохраняется в DataFrame и может быть экспортирован в CSV для дальнейшего анализа.
"""
import pandas as pd
import numpy as np
import sqlite3
...
# Шаг 4: Проверка на выброс (IQR метод)
...
# result_df.to_csv('outlier_candles.csv', index=False)
# print("Результат сохранён в 'outlier_candles.csv'")