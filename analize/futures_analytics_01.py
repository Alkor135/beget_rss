...
max_drawdown = data_df["DrawdownPct"].min()  # в долях (отриц.)
...
for idx, row in monthly.iterrows():
    ...
    comment = []
    comment.append(f"Месяц {ym}: {'+' if twr>0 else ''}{twr:.2%}")
    if flows != 0:
        comment.append(f"{'Ввод' if flows>0 else 'Вывод'}: {flows:,.0f} руб.")
